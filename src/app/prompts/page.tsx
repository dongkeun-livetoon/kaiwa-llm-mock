      ...
      <div className="grid grid-cols-12 gap-6 h-[calc(100vh-220px)]">
        ...
        <div className="col-span-3 bg-white rounded-2xl shadow-sm border border-slate-200/60 p-5 overflow-y-auto">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-xs text-slate-400">{mockPromptVersions.length} total</span>
          </div>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-white font-bold shadow-lg">
                  {selectedPrompt.version.replace('v', '')}
                </div>
                <div>
                  <h2 className="text-lg font-semibold text-slate-800">{selectedPrompt.version}</h2>
                  <p className="text-sm text-slate-500">
                    {character?.displayName} · {new Date(selectedPrompt.createdAt).toLocaleString('ja-JP')}
                  </p>
                </div>
              </div>
